...
chdb_client = chromadb.PersistentClient(path="./chroma_db")
collection = chdb_client.get_or_create_collection(name="documents")
################################################################
...
send_notification_function = {
    ...
    "description": "Sends a notification to the real Jared. Use this when:\
    1) Someone wants to get in touch, hire or collaborate.  Ask for their name and contact details first and then send them to Jared in a notification.\
    2) If you don't know information about Jared, send a notification to Jared without asking and include the question so that the real Jared can decide\
        whether to add the information or not.",
    "parameters": {
        "type": "object",
        "properties": {
            "message": {
